...
export default () => {
  ...
  const back = () =>{
	...
	const showSubmit=()=>{
		...
		Dialog.confirm({
			content: <>
				<div className='k3-confirm-title'>购买确认</div>
				<Divider />
				<div className='k3-confirm-text'>数据量值：{gameData.qishu}</div>
				<div className='k3-confirm-text'>数据购买总额：{amount*Object.keys(touzhu).length}元</div>
				<div className='k3-confirm-text'>数据购买内容：
		...
		<div className='touzhu-number' style={{'display':Object.keys(touzhu).length==0?"none":"block"}}> 
			...
			<div>
				<div  className='touzhu-number-row'  >
					<div style={{float:"left"}}>购买数据量</div>
